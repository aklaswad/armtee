...
;(($root$,$printer$) => {` }
  ...
  __footer_js () { return `})(data,printer)` }

  pushTranspiled (block) {
  ...
  translate (options={}) {
    ...
    if ( options.mode === 'function' )
      this.buf.push(this.__header_js())

    this.blocks.forEach( (b,idx) => {
      if ( options.inject && options.injectLine === idx ) {
        this.buf.push(options.inject)
      }
    ...
    if ( options.mode === 'function' )
      this.buf.push(this.__footer_js())
    return this.rawScript = this.buf
      .map( b => b
        .replaceAll('$printer$', this.printer)
...
Armtee.addMacro('FILTER', (armtee, block, args) => {
