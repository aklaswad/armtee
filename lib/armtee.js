...
class ArmteeBlock {
  ...
  parseError (error) {
    throw `Armtee template error: ${error} at ${ this.file } line ${ this.line }`
  }
  ...
  static create ( type, txt, src ) {
    switch (type) {
      ...
      case 'comment' :
        ...
        break
      case 'system' :
        return new ArmteeSystemBlock(txt, src)
        break
      default :
...
class ArmteeScriptBlock extends ArmteeBlock {
  ...
  compile (armtee, txt) {
    const ret = []
    if ( Armtee.debug ) {
      ret.push( '$printer$._trace(' + JSON.stringify(block) + ')' )
    }
    ret.push(this.txt)
    return ret
  }
...
class ArmteeMacroBlock extends ArmteeBlock {
  ...
  compile (armtee, txt) {
    if ( this.handler.compile ) {
      return this.handler.compile(armtee, this.args, this)
    }
    return
  }
}
...
-------------
    ...
    if ( Armtee.debug ) {
      ret.push(this.transpiled.push( armtee.runtimeSymbols.printer + '._trace(' + JSON.stringify(block) + ')' ))
    }
...
class ArmteeSystemBlock extends ArmteeBlock {
  static type = 'system'
}
...
-------------`)
  ...
  __function_header () {
    const script = `
Object.entries(printer.filters)
  .forEach(f => {
    String.prototype['$'+f[0]] = f[1]
  })
;((${this.runtimeSymbols.root},${this.runtimeSymbols.printer}) => {`
  ...
  translate (options={}) {
    ...
    const compiledLines = blocks.flatMap( block => block.compile(this, block.txt) )
  ...
  render (data) {
    ...
    printer.filters = Armtee.__filters
...
Armtee.addFilter( 'hey', function () { return 'hey? ' + this })
