...
-------------`)
  ...
  setUpPrinter (buf) {
    const printer = function (...args) {
      ...
      buf.push(printer.f ? printer.f(raw) : raw)
    }
    const trace = []
    printer['_trace'] = function (block) { trace.push(block) }
    printer.$ = Armtee.__filters
    printer._ = []
  ...
  render (data, options={}) {
    ...
    const buf = []
    const printer = this.setUpPrinter(buf)
    try {
...
Armtee.addMacro('FILTER', {
  compile: (armtee, args) => {
    ...
    return [`${armtee.runtimeSymbols.printer}.f = ${armtee.runtimeSymbols.printer}.$.${filterName}`]
  }
})

Armtee.addMacro('INCLUDE', {
  precompile: (armtee, args, block) => {
    if ( false && block.src.type !== 'file' ) {
    ...
    return [
      ArmteeBlock.create('script', `;((${included.runtimeSymbols.root},${included.runtimeSymbols.printer}) => {`, systemSrc),
      ...blocks,
      ArmteeBlock.create('script', `})( ${context}, ${armtee.runtimeSymbols.printer} )`, systemSrc)
    ]
  },

  compile: (armtee, args) => {
    return [`${armtee.runtimeSymbols.printer}.push()`]
  }
})
