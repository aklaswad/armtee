...
-------------
  ...
  compile (debugOption) {
    ...
    let fn
    try {
      fn = new Function(this.root, this.printer, js)
    }
    catch (e) {
      ...
      FIND: for ( const r of errorRaisers ) {
        try {
          const js = this.compileCore({ injectLine: 0, inject: r })
          new Function(js)
        }
        catch (e) {
          if ( orig !== e.message) {
      ...
      while ( true ) {
        ...
        const js = this.compileCore({ injectLine: nth, inject: raiser })
        try {
          new Function(js)
        }
        catch(e) {
